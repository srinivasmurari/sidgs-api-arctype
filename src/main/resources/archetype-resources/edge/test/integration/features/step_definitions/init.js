...
var environment = config['${projectName}-${teamName}-${artifactId}'].env;
...
console.log('&$${projectName}-${teamName}-${artifactId} api: [' +  domains[environment] + ', ' + config['${projectName}-${teamName}-${artifactId}'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['${projectName}-${teamName}-${artifactId}'].app, config['${projectName}-${teamName}-${artifactId}'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['${projectName}-${teamName}-${artifactId}'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
